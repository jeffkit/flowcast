...
test('fanOut: prepare 抛错（硬失败）→ fanOut reject 且 err.partialResults 含已完成结果', async () => {
  const flowDir = mkdtempSync(join(tmpdir(), 'flowcast-fo-partial-'))
  const okFlow = join(flowDir, 'ok.mjs')
  writeFileSync(okFlow, `// noop\n`)
  ...
  try {
    const err = await fanOut(
      [
        { name: 'task-ok', flow: okFlow },
        { name: 'task-fail-prepare', flow: okFlow },
      ],
      {
        repo: flowDir, isolate: 'none', dryRun: false, timeout: 10_000, concurrency: 1,
        prepare: async (task) => {
          if (firstPrepared) throw new Error('prepare boom')
          firstPrepared = true
        },
      },
    ).then(() => null, e => e)
    ...
    assert.ok(err.partialResults.some(r => r?.task?.name === 'task-ok'), '已完成的 task-ok 应在 partialResults 中')
  } finally { rmSync(flowDir, { recursive: true, force: true }) }
})
