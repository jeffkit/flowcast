...
test('archiveChildRun：把 worktree 内子 run 镜像回主仓，保全观测数据', () => {
  ...
  try {
    ...
    // 主仓是全新项目（无 .flowx/ 无 .flowcast/）→ flowcastDir 默认落 .flowcast/runs；
    // 源在 worktree 的旧 .flowx/runs，archiveChildRun 跨 scheme 镜像到主仓新目录。
    const mirrored = join(repo, '.flowcast', 'runs', 'drain-9-x', 'state.json')
    assert.ok(existsSync(mirrored), '子 run 应被镜像到主仓 .flowcast/runs')
