...
/**
 * Node 20 Linux 上 rimraf 内部对并发 appendFile 竞态更敏感，会抛 ENOTEMPTY。
 * 用指数退避重试，让 event loop 有机会排干 Checkpoint._logQueue 里的 appendFile。
 */
async function safeRm(dir) {
  for (let i = 0; i < 5; i++) {
    try { rmSync(dir, { recursive: true, force: true }); return } catch (e) {
      if (e.code !== 'ENOTEMPTY' || i === 4) throw e
      await new Promise(res => setTimeout(res, 20 * (i + 1)))
    }
  }
}
...
test('Checkpoint.done：async + 同步落盘 _logQueue，调用方不 await 也能拿到完整日志', async () => {
  // done/pause 是终止路径：进程退出前不显式 flushLog 就丢日志。
  // done 内部 await _logQueue 保证日志落盘后再返回，调用方可以忽略返回值。
  const dir = tempDir()
  try {
    const cp = new Checkpoint('rdone', dir)
    cp.event('gate', { name: 'test', status: 'pass' })
    // 不 await done()，模拟调用方"忘了 await"
    const p = cp.done({ summary: 1 })
    assert.ok(p && typeof p.then === 'function', 'done 应返回 Promise')
    await p
    // 同步落盘：日志应已存在（即使调用方没显式 flushLog）
    const logPath = join(dir, 'rdone', 'run.log.jsonl')
    assert.ok(existsSync(logPath), 'done() 返回后 run.log.jsonl 必须存在')
    const lines = readFileSync(logPath, 'utf8').trim().split('\n').filter(Boolean)
    assert.ok(lines.length >= 1, '至少有一条事件落盘')
    assert.equal(JSON.parse(lines[lines.length - 1]).event, 'gate')
  } finally { await safeRm(dir) }
})

test('Checkpoint.pause：async，await _logQueue 后才抛 PauseSignal', async () => {
  // pause 是终止路径：必须在 PauseSignal 抛出前完成日志落盘，
  // 否则 catch PauseSignal 退出进程时丢日志。
  const dir = tempDir()
  try {
    const cp = new Checkpoint('rpause', dir)
    cp.event('hitl', { node: 'review', action: 'prompt' })
    let pauseSignal
    try {
      await cp.pause('人工审批', { ticket: 'T-1' })
      assert.fail('pause 应该抛 PauseSignal')
    } catch (e) {
      pauseSignal = e
    }
    assert.equal(pauseSignal.name, 'PauseSignal')
    // 日志必须在抛错前已落盘
    const logPath = join(dir, 'rpause', 'run.log.jsonl')
    assert.ok(existsSync(logPath), 'pause() 抛出 PauseSignal 前 run.log.jsonl 必须存在')
    const lines = readFileSync(logPath, 'utf8').trim().split('\n').filter(Boolean)
    assert.ok(lines.length >= 2, 'pause 前至少有 event + pause 两条日志')
    const last = JSON.parse(lines[lines.length - 1])
    assert.equal(last.status, 'paused')
    assert.equal(last.reason, '人工审批')
    // state.json 也已落盘（status=paused）
    const state = JSON.parse(readFileSync(join(dir, 'rpause', 'state.json'), 'utf8'))
    assert.equal(state.status, 'paused')
    assert.equal(state.pauseReason, '人工审批')
    assert.deepEqual(state.pauseContext, { ticket: 'T-1' })
  } finally { await safeRm(dir) }
})
...
test('Checkpoint.step: sidecar 文件丢失 → 步骤重新执行（不跳过）', async () => {
  ...
  let cp, cp2
  try {
    cp = new Checkpoint('r-sidecar', dir)
    ...
    cp2 = new Checkpoint('r-sidecar', dir)
...
test('Checkpoint.pause: 抛出 PauseSignal，状态落盘为 paused', async () => {
  const dir = tempDir()
  try {
