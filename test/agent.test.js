...
// pipeline 测试已迁移到 test/concurrency.test.js
...
test('claude adapter: spawnCapture ENOENT → SpawnError（不存在的二进制路径）', async () => {
  const { SpawnError, FlowcastError } = await import('../errors.js')
  // 验证 spawnCapture 对不存在的 binary 返回 spawnError 字段（adapters.js:80 的前置条件）
  const r = await spawnCapture('/nonexistent/claude-binary-does-not-exist', [], { timeout: 2000 })
  assert.ok(r.spawnError, '不存在的 binary 应返回 spawnError')
  assert.equal(r.exitCode, -1)
  // 验证 claude adapter 用 new SpawnError(msg, spawnMsg) 包装后结构正确
  const err = new SpawnError('[claude] spawn error', String(r.spawnError))
  assert.ok(err instanceof SpawnError, 'SpawnError instanceof SpawnError')
  assert.ok(err instanceof FlowcastError, 'SpawnError instanceof FlowcastError')
  assert.equal(err.code, 'SPAWN_ERROR')
  assert.ok(err.spawnError, 'spawnError 字段应含启动失败原因')
})
