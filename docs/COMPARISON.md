...
- **flowcast**：执行单元 = 一个 **CLI 进程**（`claude`、`cursor`、`gemini`、`codex`、`recursive` 二进制……），通过 [AgentProc](https://github.com/jeffkit/agentproc) in-process executor 统一驱动（见 [AGENTPROC_MIGRATION.md](AGENTPROC_MIGRATION.md)）
