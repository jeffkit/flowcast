...
生成的 flow 受 import 白名单约束（不能直接用 `child_process`），但常需要 git。flowcast 暴露这组受控 helper：
...
::: tip 为什么不裸调 git
按 [FLOW_API](/api/) 契约，生成的 flow 需要 git/文件操作时**只能**走 flowcast 暴露的原语，不能裸调 shell。这样契约不破，且 dry-run 可空跑。
:::
...
把 worktree 内某条子 run 的状态（`state.json` / `run.log.jsonl`）镜像回主仓 `.flowcast/runs`，让看板能在主仓一处稳定读到（worktree 会被复用/清理，观测数据否则会丢）。纯保全操作，失败只告警。
