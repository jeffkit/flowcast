...
所有对外 API 都从 `@force-lab/flowx` 导出（在 `index.js` 登记）。本页是分类索引，点进各分页看完整签名。
...
L3 的 API 从 `@force-lab/flowx/orchestrator`（`orchestrator/index.js`）导出：
...
| `checkFlowxResolvable(repo)` | 预检：目标仓能否解析 `@force-lab/flowx` |
