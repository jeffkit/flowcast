...
② 跑前校验     语法（node --check）+ import 白名单（只许 @force-lab/flowx + util）+ dry-run 冒烟
③ 持久化+续跑   生成的 flow.mjs 落盘到 .flowx/runs/<run-id>/，续跑直接复用，绝不重生成
```

生成的 flow **只能** import `@force-lab/flowx`（+ `util`），只能用 [FLOW_API](/api/) 列出的原语。需要 git / 文件 / 进程操作时，只能通过 flowx 暴露的原语（`gitCommitAll`、`runFlow`…），不能裸调 `child_process` / `fs`。`validateFlow` 会拦截违规。
...
1. **预检** `checkFlowxResolvable`：目标仓必须能解析 `@force-lab/flowx`，否则毫秒级 fail-fast 并给出 `npm install` 指引。
...
手写编排（如 `flows/todo-drain.js`）与 LLM 分拆（`orchestrateMulti`）**共用 `fanOut` 这一底座**。
...
```js
import { orchestrate, orchestrateMulti, loadAgents, loadProviders } from '@force-lab/flowx'
