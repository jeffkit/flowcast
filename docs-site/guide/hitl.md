...
HITL（Human-in-the-Loop）让 flow 在关键节点**阻塞等人工决策**或**单向通知**。flowcast 的 HITL 后端是**可插拔**的：终端、企业微信，或你自己注入的任意实现。
...
```js
setHitlBackend('terminal')                          // 明确选用终端输入（默认 null，必须显式调用）
setHitlBackend('wecom', { projectName: 'flowcast' })   // 企业微信
...
terminal 是最常用的本地开发后端。**默认后端为 null**——flow 必须在启动时显式调用 `setHitlBackend`，否则 `waitForInput`/`notify` 会抛 `ConfigError`（防止 CI/子进程环境下静默挂死）。用 Node 的 `readline` 在终端阻塞读取一行输入。适合本地交互式开发。
...
```bash
flowcast run ./flows/my-flow.js --hitl wecom --project-name flowcast
```
