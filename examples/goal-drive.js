 ...
 *
 * 用法：
 *   flowcast run goal-drive --goal "<目标>" --gate "npm test" --repo . --cli claude
 *   flowcast run goal-drive --prompt-file ./goal.md --gate "npm test" --max-turns 8
 *   flowcast run goal-drive --run-id <id>            # 断点续跑（同 run）
 *   flowcast run goal-drive --dry-run                # 假执行器 + 假质量门，空跑骨架
 *
